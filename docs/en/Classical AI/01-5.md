...
- MCTS: Note that MCTS does not have to retain approximate values/policies from one action selection to the next, since it updates the value estimates of nodes along the path to reflect the win percentage of simulations passing through that node. Or: Unlike standard MC in RL that is used to learn value functions and policies in unknown environments, MCTS is used for decision-making in known environments (e.g., games) and it does not require complete episodes; instead, it simulates rollouts from a current state to explore the best moves. What is updated is the win/loss counts/statistics up the tree.
