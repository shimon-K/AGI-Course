...
- If $A$ and $B$ are mutually exlusive events (and $P(A),P(B)>0$), i.e. $P(A\bigcap B) = 0 \ne P(A)P(B)$, which means $A,B$ are not independent (i.e. dependent). And vice versa: $P(A\bigcap B) = P(A)P(B) \ne 0$.
...
- Note that there is also _“Moment generating function”_, defined as $M_X(t) = E[e^{tX}]$, whose derivative is the moments. This is similar to the relation between cdf and pdf.
